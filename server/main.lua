...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle2', function (source, cb, vehicleModel)
	local xPlayer     = ESX.GetPlayerFromId(source)
	local vehicleData = nil
	...
	if xPlayer.getMoney() >= vehicleData.price * 0.05 then
		xPlayer.removeMoney(vehicleData.price * 0.05)
		cb(true)
	else
		cb(false)
	end
